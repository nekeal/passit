...
class NewsManager(Manager):  # type: ignore
    ...
    def get_by_profile(self, profile: UserProfile) -> 'QuerySet[News]':
        member_of_field_age_group = Membership.objects.filter(profile=profile).values('field_age_group_id')
        return self.get_queryset().filter(Q(field_age_group_id__in=member_of_field_age_group))
