...
def custom_user_list_view():
    return CustomUserViewSet.as_view(
        {"put": "set_default_field_age_group", "post": "create"}
    )


class TestCustomUserViewSet:
    def test_set_default_field_age_group_action2(
        self, student1, api_rf, custom_user_list_view
    ):
        default_membership = student1.memberships.get()
        url = "/api/auth/users/set_default_fag/"
        non_default_membership: Membership = MembershipFactory(
            profile=student1, is_default=False
        )
