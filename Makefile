...
generate-statics:
	docker build --target frontend-builder -t tmp-react-build .
	docker run -it --name tmp-react-build tmp-react-build bash
	docker cp tmp-react-build:/app/build $(shell pwd)/$(PROJECT_NAME)/frontend/
	docker cp tmp-react-build:/app/config $(shell pwd)/$(PROJECT_NAME)/frontend/
	docker rm -f tmp-react-build
