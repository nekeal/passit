...
class TestEventViewSet:
    ...
    def test_can_create_event_without_subject_group(self, event_data, api_rf, event_list_view, user_profile1):
        ...
        assert not event_data.get('subject_group')
        assert response.status_code == status.HTTP_201_CREATED

    def test_can_create_event_with_subject_group(self, event_with_subject_group_data, api_rf, event_list_view, user_profile1):
        request = api_rf.post(reverse('api:event-list'), data=event_with_subject_group_data)
        force_authenticate(request, user_profile1.user)
        response = event_list_view(request)
        assert response.status_code == status.HTTP_201_CREATED

    def test_cant_create_event_without_field_age_group(self, event_data, api_rf, event_list_view, user_profile1):
