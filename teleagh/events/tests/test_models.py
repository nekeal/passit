...
class TestEventManager:
    ...
    def test_number_of_queries_in_get_by_profile(self, monkeypatch, django_assert_num_queries):
    ...
    def test_get_by_profile_result(self, monkeypatch):
        monkeypatch.setattr(MembershipQuerySet, 'values_list', mock.Mock(return_value=[11, 12]))
        profile = UserProfileFactory.build()
        expected = {EventFactory(field_age_group__id=11), EventFactory(field_age_group__id=12)}
